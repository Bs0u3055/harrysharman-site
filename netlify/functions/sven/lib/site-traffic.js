...
async function saveAndSendTrafficReport(config, options = {}) {
  const now = options.now || new Date();
  ...
  const chatId = config.trafficReportTelegramChatId || config.adminTelegramChatId;
  if (chatId && config.telegramBotToken) {
    await sendMessage(config, chatId, report.slice(0, 3900));
  }
  return report;
}
